...
// Check if the form has been submitted or closed before showing the popup
if (document.cookie.indexOf('formSubmitted=true') === -1 && document.cookie.indexOf('formClosed=true') === -1) {
  window.addEventListener('scroll', () => {
    if (popupShown) return; // Prevent multiple triggers

    // Total scrollable height minus the viewport height
    const scrollableHeight = document.documentElement.scrollHeight - window.innerHeight;

    // Current scroll position
    const currentScroll = window.scrollY;

    // Check if the user has scrolled 55% of the total scrollable height
    if (currentScroll >= scrollableHeight * 0.55) {
      showPopup();
    }
  });
  ...
  function resetInactivityTimer() {
    ...
    if (!popupShown) {
      inactivityTimer = setTimeout(showPopup, 5000); // 5 seconds
    }
  }
...
function closePopup() {
  ...
  setCookie('formClosed', 'true', 1); // Set cookie when the form is closed
}
...
$('button.form_submit_popup').click(function (e) {
  e.preventDefault();
    ...
    grecaptcha.execute(siteKey, { action: 'submit' }).then(function (token) {
      if (frmSubmit(myform)) {
        ...
        var data = {
          ...
          contactNumber: $.trim(
            $(myform).find('.countryCode').val() + ' ' + $(myform).find('input[name="phoneNumber"]').val()
          ),
        ...
        if (
          ...
          data.restaurantName != '' &&
          data.email != '' &&
          data.yourCountry != 'Select Country' &&
          data.yourCity != '' &&
          data.contactNumber.length >= 9
        ) {
          $('div#loading').css('display', 'block');
          $.post(
            'https://website-service.dashboards.greenwheels.com/api/v1/lt-website-service/restaurant/create',
            {
              name: data.yourName,
...
$('.cross-removel-icon').click(function () {
  closePopup();
});
