  $("button.form_submit_new").click(function(e){
    e.preventDefault();
      ...
      var data = {
        ...
        contact_number: $.trim($('input[name="contact_number"]').val()),
        email_id: $.trim($('input[name="email_id"]').val()),
        send_message: $('textarea#send_message').val()
      }

      // validate and not send post request untill and unless data is validated
      if((data.your_name != '') && (data.contact_number != '') && (data.email_id != '') && (data.send_message != '')){
         if (ValidateEmail(data.email_id)) {
$.post("https://website-service.dashboards.greenwheels.com/api/v1/lt-website-service/restaurant/contact",
           {
             name: data.your_name,
           ...
           function (data, status) {
              ...
              ga('send', 'event', 'Write To Us', 'submit');  
            if(data.id){
...
/*for home page*/
var whitebgheader = $('#landing-contact').height();
...
$(document).ready(function(){
  $("a[href='#form-section']").click(function(e) {
    
    e.preventDefault();
    var targetOffset = $('#form-section').offset().top-74;
        $('html,body')
...
$(document).ready(function(){
  $("a[href='#form-section']").click(function(e) {
    ...
    e.preventDefault();
    var targetOffset = $('#form-section').offset().top-75;
        $('html,body')
        .animate({scrollTop: targetOffset}, 1000);
...
function initialize() {
 ...
 // greenwheels Location pointers
      ...
      ['greenwheels Gurugram ', 28.5011141,77.0868464],
    ...
    var infoWindowContent = [
       ['<div class="info_content">' +
        '<h3>greenwheels Gurugram</h3>' +
        '<p>1st floor, 445, Udyog Vihar Phase V, Gurugram, Haryana 122008</p>' +        '</div>'],
    ...
    var image = { url: 'images/about/greenwheels-aboutus-location.png', };
